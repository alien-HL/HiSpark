...
def get_split_merge(img):
    ...
    # 获取上半部分，设定不重叠的区间
    img_upper = img[0:int(1 / 2 * h), :]

    # 获取下半部分
    img_lower = img[int(1 / 2 * h):, :]

    # 检查大小
    print("Upper image size:", img_upper.shape)
    print("Lower image size:", img_lower.shape)

    # 缩放上半部分至与下半部分相同的尺寸
    img_upper = cv2.resize(img_upper, (img_lower.shape[1], img_lower.shape[0]))

    # 再次检查缩放后的尺寸
    print("Resized upper image size:", img_upper.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='/home/nicta100-s26/ai/sample/ocr/crnn_plate_recognition/saved_model/best.pth', help='model.pt path(s)')
    # parser.add_argument('--image_path', type=str, default='images/tmp6FC6.png', help='source')
    parser.add_argument('--image_path', type=str, default='/home/nicta100-s26/ai/sample/ocr/crnn_plate_recognition/images/test.jpg', help='source')
